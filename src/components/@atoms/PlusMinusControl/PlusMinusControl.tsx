  ...
  (
    ...
    return (
      <Container $highlighted={highlighted}>
        <Button type='button' onClick={incrementHandler(-1)} disabled={minusDisabled}><MinusIcon /></Button>
        <Label $highlighted={highlighted}>{t(`unit.${unit}`, { count: value })}</Label>
        <Button type='button' onClick={incrementHandler(1)} disabled={plusDisabled}><PlusIcon /></Button>
        <VisuallyHidden>
          <input
            type='number'
            {...props}
